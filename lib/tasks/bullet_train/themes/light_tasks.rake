  ...
  namespace :themes do
    namespace :light do
      ...
      task :eject, [:destination] => :environment do |task, args|
        BulletTrain::Themes::Light::Application.eject_theme(get_theme_name_from_task(task), args[:destination])
      end

      desc "Publish your custom theme theme as a Ruby gem."
      task :release, [:theme_name] => :environment do |task, args|
        BulletTrain::Themes::Light::Application.release_theme(get_theme_name_from_task(task), args)
      end

      desc "List view partials in theme that haven't changed since ejection from \"Light\"."
      task :clean, [:theme] => :environment do |task, args|
        BulletTrain::Themes::Light::Application.clean_theme(get_theme_name_from_task(task), args)
      end
